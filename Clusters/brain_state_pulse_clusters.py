'''
Build sensory/pulse-related cell clusters: select state-pulse-filtered
cells per fish, align their dFF to probe/recovery-swim trials, then
cluster the resulting dynamics (agglomerative + DBSCAN on cell location)
and save the per-fish cluster labels to fsensory_cluster_label.npy.
'''
...
np.savez(cluster_folder+'cell_list_dynamics_early_pulse.npz', \
...
save_root = df.iloc[n_animal]['save_dir']+'/'
np.save(save_root + 'fsensory_cluster_label.npy', label_sub)