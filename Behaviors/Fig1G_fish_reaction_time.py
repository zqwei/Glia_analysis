...
df = pd.read_csv('/groups/ahrens/home/weiz/Projects/Glia_analysis/Datalists/Behavioral_ephys.csv')
...
ind_list = np.array(ind_list)[rt_frac[:,0]>0.35]
...
print('non-replay CL vs OL:', wilcoxon(rt_mean[~replay_exp, 0], rt_mean[~replay_exp, 1]))
print('replay CL vs OL:', wilcoxon(rt_mean[replay_exp, 0], rt_mean[replay_exp, 1]))
print('num non-replay fish:', (~replay_exp).sum())
print('num fish total:', ind_list.shape[0])

(df.iloc[ind_list]).to_csv('Fig1G_fish.csv')
